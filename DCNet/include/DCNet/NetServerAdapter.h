...
/// @brief 节点服务化服务实例句柄（registerDcNetServerAdapter 返回）。
...
/// @brief 节点服务化（Serve-a-Node）装配描述（DESIGN.md §3.6）。
struct DcNetServerAdapterDesc {
	std::string engineType;                    ///< 被驱动执行的本地节点引擎类型（须已注册）
	std::string localModelRef;                 ///< 本地模型标识（ADR-7：本地路径语义，不用 modelPath——其全局约定为远端端点）
...
/// @brief 注册并启动一个「节点服务化」监听端（M-server）。
///
/// 请求处理路径（与本地执行完全同管线，保证图级语义无差别）：
///   decodeRequest → EngineRegistry::createNode（一请求一节点实例）→
///   setInput → tryExecute → collectOutputs → encodeResponse → 200；
///   失败经 wireStatusFor 逆向映射为 wire 状态（DESIGN.md §6.1）。
