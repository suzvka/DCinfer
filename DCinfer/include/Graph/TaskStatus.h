...
/// 经 getOutput / getOutputTensor 按 (nodeName, portName) 取出，
/// 存活至下一次同 taskId 的 submit 或 releaseTask()。
struct TaskResult {
	TaskStatus status = TaskStatus::Unknown; ///< 终止状态（超时未终止则为 Running）
