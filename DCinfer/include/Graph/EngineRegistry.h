...
public:
	...
	void*       get()       { return _ptr.get(); }
	const void* get() const { return _ptr.get(); }
...
public:
	...
	template<typename T>
	EngineInstance(std::shared_ptr<T> engine, const EngineDescriptor* desc)
		: _engine(std::move(engine)), _desc(desc) {}

	void*       get()       { return _engine.get(); }
	const void* get() const { return _engine.get(); }
...
public:
	...
	EngineInstance* getOrCreateEngine(
		const std::string& engineType,
		const std::string& modelPath);
...
private:
	...
	static std::string _makeEngineKey(const std::string& engineType,
	                                  const std::string& modelPath);
