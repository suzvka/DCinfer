...
/// 分类法保留在产生它的子系统（领域分类由子系统自定义），核心只透传。
/// 典型用例：远端报文结构异常在子系统内归类为细分错误码（如 RemoteMalformed），
/// 对核心仅表现为 ExecutionFailed + Diagnostic{domain, code}。
struct Diagnostic {
	std::string domain;  ///< 诊断域（小写标识符，由子系统定义）；空串表示无领域诊断
