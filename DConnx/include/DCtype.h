...
namespace DC {
    ...
    [[nodiscard]] std::size_t getSize(const T&) {
        return GlobalRegistry::instance().getRegistry<Enum>().template getSize<T>();
    }
    ...
    [[nodiscard]] std::size_t getSizeOr(const T&, std::size_t fallback) {
        return GlobalRegistry::instance().getRegistry<Enum>().template getSizeOr<T>(fallback);
    }

    /// @brief 查询与类型 T 关联的注册大小，如果未找到则返回备用值。
    /// @tparam Enum 目标枚举类型。
    /// @tparam T 要查询的类型。
    /// @param fallback 如果未找到映射，则返回此值。
    /// @return 如果找到，则返回对应的大小；否则返回 fallback。
    template<class Enum, class T>
    [[nodiscard]] std::size_t getSizeOr(std::size_t fallback) {
        return GlobalRegistry::instance().getRegistry<Enum>().template getSizeOr<T>(fallback);
    }
