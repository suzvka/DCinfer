...
Not DAG 的 C++20 推理管线编排器，面向端云协同 AI 的策略驱动推理运行时。

DCinfer 不是推理引擎，而是支持分布式异构多端推理的编排层。它让 AI 应用在本地、云端、PC、局域网设备和不同推理引擎之间自由放置模型节点，同时把网络、调度、数据传输、引擎生命周期等复杂度隔离在 Runtime 层。

提供节点编排、多引擎热插拔、协程驱动的异步数据流传播能力，适用于云端混合推理、多模型推理管道、预处理/后处理编排、自定义算子(概念)集成等场景。
---

## 核心特性

- **Not DAG 拓扑编排** 
-- 节点 + 端口级边连接，支持复杂推理管道建模
-- 支持成环
-- 用节点/导线/执行器概念自然建模高并发推理模式
- **原生面向并发** 
-- 可配置线程池Compute、Operator、System
-- 运行时通过数据驱动，所有算子乱序执行，可最大化硬件利用率
- **统一安全张量** 
-- 编译期 Schema 定义 + 运行时 类型/形状 校验
-- Tensor[a][b][c] 链式视图索引编辑，类numpy
- **无外部依赖** 
-- 核心库为 static lib，零第三方依赖
