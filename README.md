...
DCinfer 不是推理引擎，而是推理执行自由度的编排层。它让 AI 应用在本地、云端、PC、局域网设备和不同推理引擎之间自由放置模型节点，同时把网络、调度、数据传输、引擎生命周期等复杂度隔离在 Runtime 层。

提供节点编排、多引擎热插拔、协程驱动的异步数据流传播能力，适用于云端混合推理、多模型推理管道、预处理/后处理编排、自定义算子集成等场景。
---
